...
const NameCheck = (props) => {
    ...
    useEffect(async () => {
        dispatch(setToolbarHeader("Name Check Report"));
    ...
    useEffect(() => {
        if (!_.isEmpty(ptrDetails) || !_.isEmpty(ptrAlphaRecords)) {
            const docData = getFilteredDocumentsFromPTR(ptrDetails, ["NAME_CHECK",], showAllDoc)
            ...
            const alphaDocData = showAllDoc ? ptrAlphaRecords : filterDocuments(ptrAlphaRecords, ["NAME_CHECK",])
    ...
    return (
        <Box sx={{margin: "30px"}}>
            ...
            {dataToPrint && <PrintModal
                ...
                subTitle="PinPointGuam: Name Check List"
