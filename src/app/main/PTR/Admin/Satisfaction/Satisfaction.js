...
const Satisfaction = (props) => {
    ...
    useEffect(async () => {
        dispatch(setToolbarHeader("Satisfaction Report"));
    ...
    useEffect(() => {
        if (!_.isEmpty(ptrDetails) || !_.isEmpty(ptrAlphaRecords)) {
            const docData = getFilteredDocumentsFromPTR(ptrDetails, ["SATISFACTION",], showAllDoc)
    ...
    return (
        <Box sx={{margin: "30px"}}>
            ...
            {dataToPrint && <PrintModal
                ...
                subTitle="PinPointGuam: Satisfaction List"
