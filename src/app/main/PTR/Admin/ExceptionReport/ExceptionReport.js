...
import {
    ...
    customDataSearch,
    dateSortCompare, docSortCompare,
    filterDocuments,
...
const ExceptionReport = (props) => {
    ...
    useEffect(async () => {
        dispatch(setToolbarHeader('Document Audit Report Screen'));
    ...
    useEffect(() => {
        if (!_.isEmpty(ptrDetails) || !_.isEmpty(ptrAlphaRecords)) {
            const docData = getFilteredDocumentsFromPTR(ptrDetails, ["EXCEPTION"], showAllDoc);
            ...
            const alphaDocData = showAllDoc ? ptrAlphaRecords : filterDocuments(ptrAlphaRecords, ["EXCEPTION"])
    ...
    const options = useMemo(() => ({
        ...
        selectableRows: "none",
        downloadOptions: {
            filename: 'Document-Audit-Report.csv',
            separator: ','
        },
        print: false,
        customToolbar: () => (
    ...
    return (
        <Box sx={{margin: '30px'}}>
            ...
            <MUIDataTable
                title={<Typography variant="h6">PTR Document Audit Report {isLoading &&
            ...
            {dataToPrint && <PrintModal
                ...
                subTitle="PinPointGuam: Document Audit Report"
